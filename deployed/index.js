...
app.get('/account/login/:email/:password', function (req, res) {
    ...
    var getRecord = findRecord(req.params.email);
    if (getRecord){
        // second check to see if password is correct & if so write transaction & return record
...
app.get('/account/deposit/:email/:amount', function (req, res) {
    ...
    if (getRecord){
        ...
        db.get('accounts')
          ...
          .write();
        console.log(`Deposit Successful - Account [${ req.params.email }] now has a balance of [${ getRecord.balance }]`);
        res.send(`Deposit Successful - Account [${ req.params.email }] now has a balance of [${ getRecord.balance }]`);
    }
    // if account does not exist then return error message
    else{
        console.log(`Account [${ req.params.email }] does not exist!`);
        res.send(`Account [${ req.params.email }] does not exist!`);
    }
});
    ...
    if (getRecord){
        ...
        db.get('accounts')
          ...
          .assign({ balance: getRecord.balance,
                    transactions: getRecord.transactions })
          .write();
        console.log(`Withdrawal Successful - Account [${ req.params.email }] now has a balance of [${ getRecord.balance }]`);
        res.send(`Withdrawal Successful - Account [${ req.params.email }] now has a balance of [${ getRecord.balance }]`);
    }
    // if account does not exist then return error message
    else{
        console.log(`Account [${ req.params.email }] does not exist!`);
...
app.get('/account/all', function (req, res) {
    var getAllRecords = (db.get('accounts').value());
